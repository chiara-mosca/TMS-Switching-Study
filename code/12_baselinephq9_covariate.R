# ══════════════════════════════════════════════════════════════  
# 3-PANEL PHQ-9 SLOPE PLOT — WITH BASELINE PHQ-9 COVARIATE  
# ══════════════════════════════════════════════════════════════
...
# ── 5. COMPUTE BASELINE PHQ-9 (visit 0 or first observation) ──  
baseline_scores <- g3 %>%  
  group_by(study_id) %>%  
  arrange(phq9_date) %>%  
  slice(1) %>%  
...
cat("\nBaseline PHQ-9 computed for", nrow(baseline_scores), "patients\n")

# Merge into main dataset  
g3 <- g3 %>%  
  left_join(baseline_scores, by = "study_id")

# Check baseline by group  
cat("\n=== Baseline PHQ-9 by Group ===\n")  
g3 %>%  
  select(study_id, panel_group, baseline_phq9) %>%  
  distinct(study_id, .keep_all = TRUE) %>%  
  mutate(group_label = case_when(  
    panel_group == "stay_p1" ~ "iTBS-Only",  
    TRUE ~ "Switchers"  
  )) %>%  
  group_by(group_label) %>%  
  summarise(  
    n = n(),  
    mean = round(mean(baseline_phq9, na.rm = TRUE), 1),  
    sd = round(sd(baseline_phq9, na.rm = TRUE), 1),  
    .groups = "drop"  
  ) %>%  
  print()
...
# ── 7. Fit HLM — WITH BASELINE PHQ-9 COVARIATE ──  
cat("\nFitting HLM with baseline PHQ-9 covariate...\n")
...
cat("\nPHQ-9 SLOPE PER GROUP (controlling for baseline PHQ-9):\n")  
cat(sprintf("  Stayers (never switched)  : %+.3f /week\n", slope_stay_p1))  
cat(sprintf("  Switchers phase 1         : %+.3f /week\n", slope_switch_p1))  
cat(sprintf("  Switchers phase 2         : %+.3f /week\n", slope_switch_p2))  
cat(sprintf("  Baseline PHQ-9 effect     : %+.3f (p = %.4f)\n",  
            fe["baseline_phq9"],  
            summary(m3)$coefficients["baseline_phq9", "Pr(>|t|)"]))
...
# ══════════════════════════════════════════════════════════════  
# 10. PUBLICATION-READY 3-PANEL PLOT  
# ══════════════════════════════════════════════════════════════
...
combined <- p1 + p2 + p3 +  
  plot_layout(ncol = 3) +  
  plot_annotation(  
    title = "PHQ-9 Slope Trajectory by Protocol Group (Adjusted for Baseline PHQ-9)",  
    subtitle = sprintf(  
      paste0("Controlling for baseline severity | ",  
             "Within switchers: phase 2 \u2212 phase 1 %+.2f/wk, p = %.3f | ",  
             "Between: stayers \u2212 phase 1 %+.2f/wk, p = %.3f"),  
...
ggsave("/Users/chiara/Documents/TMS-Switching-Study/phq9_slopes_3panel_baseline_adjusted.png",  
       combined, width = 17, height = 6, dpi = 300)  
ggsave("/Users/chiara/Documents/TMS-Switching-Study/phq9_slopes_3panel_baseline_adjusted.pdf",  
       combined, width = 17, height = 6)

cat("\n\u2713 Saved: phq9_slopes_3panel_baseline_adjusted.png\n")  
cat("\u2713 Saved: phq9_slopes_3panel_baseline_adjusted.pdf\n")

# ══════════════════════════════════════════════════════════════  
# 11. COMPARISON: ORIGINAL vs BASELINE-ADJUSTED  
# ══════════════════════════════════════════════════════════════

cat("\n══════════════════════════════════════════════════════════════\n")  
cat("  COMPARISON: ORIGINAL vs BASELINE-ADJUSTED\n")  
cat("══════════════════════════════════════════════════════════════\n\n")

# Run original model (without baseline) for comparison  
m3_orig <- lmer(  
...
fe_orig <- fixef(m3_orig)

slope_stay_orig    <- fe_orig["weeks_in_phase"] + fe_orig["weeks_in_phase:panel_groupstay_p1"]  
slope_switch1_orig <- fe_orig["weeks_in_phase"]  
slope_switch2_orig <- fe_orig["weeks_in_phase"] + fe_orig["weeks_in_phase:panel_groupswitch_p2"]

cat(sprintf("%-30s %12s %12s\n", "", "Original", "Adjusted"))  
cat(paste(rep("-", 56), collapse = ""), "\n")  
cat(sprintf("%-30s %+12.3f %+12.3f\n", "Stayers slope (/wk)",  
            slope_stay_orig, slope_stay_p1))  
cat(sprintf("%-30s %+12.3f %+12.3f\n", "Switchers P1 slope (/wk)",  
            slope_switch1_orig, slope_switch_p1))  
cat(sprintf("%-30s %+12.3f %+12.3f\n", "Switchers P2 slope (/wk)",  
            slope_switch2_orig, slope_switch_p2))  
cat(paste(rep("-", 56), collapse = ""), "\n")

# Original contrasts  
vcov_orig <- vcov(m3_orig)  
fe_names_orig <- names(fe_orig)

slope_contrast_orig <- function(pos, neg = NULL) {  
  r <- matrix(0, nrow = 1, ncol = length(fe_orig))  
  colnames(r) <- fe_names_orig  
  r[1, pos] <- 1  
  if (!is.null(neg)) r[1, neg] <- -1  
  est  <- as.numeric(r %*% fe_orig)  
  se   <- as.numeric(sqrt(r %*% vcov_orig %*% t(r)))  
  tval <- est / se  
  df_res <- df.residual(m3_orig)  
  pval <- 2 * pt(abs(tval), df = df_res, lower.tail = FALSE)  
  return(c(estimate = est, p = pval))  
}

within_orig <- slope_contrast_orig("weeks_in_phase:panel_groupswitch_p2")  
btw_p1_orig <- slope_contrast_orig("weeks_in_phase:panel_groupstay_p1")

cat(sprintf("\n%-30s %12s %12s\n", "Contrast", "Original p", "Adjusted p"))  
cat(paste(rep("-", 56), collapse = ""), "\n")  
cat(sprintf("%-30s %12.4f %12.4f\n", "Within (P2-P1)",  
            within_orig["p"], within_c["p"]))  
cat(sprintf("%-30s %12.4f %12.4f\n", "Between (Stay-P1)",  
            btw_p1_orig["p"], btw_p1["p"]))  
cat(paste(rep("-", 56), collapse = ""), "\n")

cat(sprintf("\nBaseline PHQ-9 covariate: \u03b2 = %+.3f, p = %.4f\n",  
            fe["baseline_phq9"],  
            summary(m3)$coefficients["baseline_phq9", "Pr(>|t|)"]))

# Conclusion  
cat("\n=== CONCLUSION ===\n")  
if (within_c["p"] < 0.05 & btw_p1["p"] < 0.05) {  
  cat("Both within-subject and between-subject contrasts remain\n")  
  cat("significant after controlling for baseline PHQ-9 severity.\n")  
  cat("The slope differences are NOT driven by baseline differences.\n")  
} else if (within_c["p"] < 0.05) {  
  cat("Within-subject contrast remains significant.\n")  
  cat("Between-subject contrast is no longer significant,\n")  
  cat("suggesting baseline severity partially explains that difference.\n")  
} else {  
  cat("Results are attenuated after baseline adjustment.\n")  
}
