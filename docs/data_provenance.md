...
- **Location:** Not in repository (contains clinical data — contact Chiara Mosca for access)  
...
| phase | character | Treatment phase relative to switch | "pre", "post" |  
...
| group | character | Analytic group assignment | "iTBS_only", "switcher" |
...
| category | character | Original clinical category | "Left-Sided Only", "Switched" |  
| consultation_phq9 | integer | Pre-treatment consultation PHQ-9 | 5–27 (some missing) |  
| change | integer | PHQ-9 change (baseline minus endpoint) | -11 to 22 |  
...
| reason | character | Exclusion reason | See table below |
...
|---|---|---|---|  
| N | 85 | 41 | 44 |  
| Remitters | TBD | TBD | TBD |  
| Responders | TBD | TBD | TBD |  
| Non-Responders | TBD | TBD | TBD |
...
| Outcome | Definition |  
|---|---|  
| Remitter | Endpoint PHQ-9 < 5 |  
| Responder | ≥50% reduction in PHQ-9 from baseline |  
...
| phq_scores_clean.xlsx | /data/initial_cleaning/ | Yes |  
| patients_clean.xlsx | Local only | No (demographics) |  
| tms_clean_longitudinal.xlsx | /data/intermediate | Yes |  
| tms_clean_outcomes(in).csv | /data/intermediate | Yes |  
| analytic_sample_long.xlsx | /data/final/ | Yes |  
| analytic_sample_summary.xlsx | Local only | No (demographics) |  
| excluded_patients.xlsx | /data/final/ | Yes |  
| 01_analytic_sample.R | /code/ | Yes |  
